      ...
      {open && (
        <div className="px-5 pb-5 grid gap-4 border-t border-border">
          <div className="mt-4 grid gap-4">
            {fields.map((f) => <TextField key={f} label={FIELD_LABELS[f] ?? f} value={round[f] ?? ""} onChange={(v) => updateField(f, v)} />)}
          </div>
            ...
            <div className="grid gap-2">
              ...
              {round.options.map((opt: any, oi: number) => (
                <div key={opt.id} className="flex items-center gap-2">
                  ...
                  {opt.is_correct && <span className="text-xs text-portal">✓ верный</span>}
                </div>
              ))}
            </div>
          )}
                  ...
                  {q.options?.map((opt: any, oi: number) => (
                    <div key={opt.id} className="flex items-center gap-2">
                      ...
                      <input type="text" value={opt.label} onChange={(e) => updateOption(qi, oi, e.target.value)}
                        className="flex-1 bg-muted border border-border rounded-lg px-3 py-2 text-sm focu
$part2 = @'
...
export default function AdminScenarioEdit() {
  ...
  useEffect(() => {
    ...
    supabase.from("scenarios").select("*").eq("id", scenarioId).single().then(({ data }) => {
      ...
      setTitle(data.title); setDescription(data.description); setPriceRub(data.price_rub ?? 250);
  ...
  if (loading || pageLoading) return <div className="min-h-screen bg-background flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-portal" /></div>;
    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <main className="flex-1 container max-w-4xl py-8">
        ...
        <div className="flex gap-1 mb-6 border-b border-border">
          {[{id:"basic",label:"Основное"},{id:"intro",label:"Вступление"},{id:"rounds",label:"Раунды (" + (partyGame?.rounds?.length ?? 0) + ")"},{id:"endings",label:"Концовки"}].map((t) => (
            <button key={t.id} onClick={() => setTab(t.id as any)}
              className={"px-4 py-2 text-sm font-mono uppercase tracking-wider transition-colors " + (tab === t.id ? "text-portal border-b-2 border-portal" : "text-muted-foreground hover:text-foreground")}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === "intro" && partyGame && (
          <div className="grid gap-4">
            <TextField label="Ситуация (общий экран)" value={partyGame.intro.situation} onChange={(v) => setPartyGame({ ...partyGame, intro: { ...partyGame.intro, situation: v } })} />
        ...
        {tab === "rounds" && partyGame && (
          <div className="grid gap-3">
            {partyGame.rounds.map((round, i) => <RoundEditor key={round.id} round={round} index={i} onChange={(r) => updateRound(i, r)} />)}
          </div>
        )}

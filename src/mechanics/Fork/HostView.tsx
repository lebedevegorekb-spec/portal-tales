...
export function ForkHost({ round, submissions, playerCount, onAdvance }: MechanicViewProps<ForkRound>) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground scanlines flex flex-col items-center justify-center p-8">
      <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground mb-4">Round - Fork</p>
      ...
      <p className="text-sm text-muted-foreground mb-2">Voted: {totalVotes} / {playerCount}</p>
      <button
      ...
      >
        {totalVotes > 0 ? "Show result (" + totalVotes + "/" + playerCount + ")" : "Waiting for votes..."}
      </button>
    </div>
  );
}